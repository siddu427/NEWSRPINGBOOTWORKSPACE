	...
	public ResponseEntity<String> saveProduct(@RequestBody Product product) {
		...
		try {
			...
			if (product.getProdId() != null && service.isProductExist(product.getProdId())) {
				resp = new ResponseEntity<String>("Given Id '" + product.getProdId() + "' Data already exist",
						HttpStatus.BAD_REQUEST);

			} else { // Product id not exist
				Integer id = service.saveProduct(product);
				resp = new ResponseEntity<String>("Product '" + id + "' created Successfully!", HttpStatus.OK // 200
				// HttpStatus.CREATED //201
				);
			}

		} catch (Exception e) {
			resp = new ResponseEntity<String>("Unable to Save Product", HttpStatus.INTERNAL_SERVER_ERROR // Our App Got
																											// Exception
			);
			e.printStackTrace();
		}
	...
	public ResponseEntity<?> getOneProduct(@PathVariable Integer id) {
		...
		try {
			...
			if (opt.isPresent()) { // if Product exist
				...
				resp = new ResponseEntity<String>("Product '" + id + "' Not exist!", HttpStatus.BAD_REQUEST);
			}
	...
	public ResponseEntity<String> updateProduct(@RequestBody Product product) {
		...
		try {
			...
			if (product.getProdId() != null && service.isProductExist(product.getProdId())) {
				...
				resp = new ResponseEntity<String>("Product '" + product.getProdId() + "' not exist!",
						HttpStatus.BAD_REQUEST);
			}
	...
	public ResponseEntity<String> removeProduct(@PathVariable Integer id) {
		...
		try {
			...
			if (service.isProductExist(id)) {
				...
				resp = new ResponseEntity<String>("Product '" + id + "' not exist",
